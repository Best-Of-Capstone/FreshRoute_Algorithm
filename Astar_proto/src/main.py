...
def route_to_format(result, target_count, start, start_near, end, end_near):
    ...
    for i in range(target_count):
        ...
        tmp_duration = round(tmp_dist / 833, 1) \
                             if public_route[0][2] == "subway" else round(tmp_dist / 333, 1)
        tmp_type, tmp_description = "탑승", "지하철 탑승" \
            if public_route[0][2] == "subway" else "버스 탑승"
        tmp_step_dict = {
            "distance": tmp_dist,
            "duration": tmp_duration,
            "type": tmp_type,
            "name": tmp_description,
            "wayPoints": [0, 0] # Have to be implemented
        }
        ...
        for j in range(1, len(public_route)):
            ...
            tmp_dist = round(haversine(tmp_long, tmp_lat, public_route[j][1][1], public_route[j][1][0]) / 1000, 1)
            tmp_duration = round(tmp_dist / 0.833, 1) \
                if public_route[j][2] == "subway" else round(tmp_dist / 0.333, 1)
            tmp_type, tmp_description = "탑승", "지하철 탑승" \
                if public_route[j][2] == "subway" else "버스 탑승"
            tmp_step_dict = {
                "distance": tmp_dist,
                "duration": tmp_duration,
                "type": tmp_type,
